...
export async function checkRunnerDetails(id: string): Promise<GetRunnerDetails> {
	return await fetch(
		`http://api.paiza.io/runners/get_details?id=${encodeURIComponent(id)}&api_key=guest`
	).then((r) => r.json());
}

export async function checkRunnerStatus(id: string): Promise<CreateRunnerResponse> {
	return await fetch(
		`http://api.paiza.io/runners/get_status?id=${encodeURIComponent(id)}&api_key=guest`
	).then((r) => r.json());
}

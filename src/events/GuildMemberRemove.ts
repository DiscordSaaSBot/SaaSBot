...
export default new Event({
	...
	async handler(member: GuildMember): Promise<void> {
		logger.info(
			`The user ${member.user.tag} has left the server ${member.guild.name} with ID ${member.guild.id} on ${member.joinedAt}, and their account was created on ${member.user.createdAt}`
		);
